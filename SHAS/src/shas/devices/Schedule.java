//-------------------------------------------------------------------------------------------------------//
//Schedule class
//SHAS 
//Author Abu Hassan
// March 4, 2013
//-------------------------------------------------------------------------------------------------------//


package shas.devices;

import java.util.Date;

public class Schedule {

	private Date dtfrom;
	public Date getDtfrom() {
		return dtfrom;
	}

	public void setDtfrom(Date dtfrom) {
		this.dtfrom = dtfrom;
	}

	public Date getDtto() {
		return dtto;
	}

	public void setDtto(Date dtto) {
		this.dtto = dtto;
	}

	public Integer getLevel() {
		return level;
	}

	public void setLevel(Integer level) {
		this.level = level;
	}

	public Integer getDuration() {
		return duration;
	}

	public void setDuration(Integer duration) {
		this.duration = duration;
	}

	public Integer getStatus() {
		return status;
	}

	public void setStatus(Integer status) {
		this.status = status;
	}

	private String sprinklerid;  
	private Date dtto;
	private Integer level;
	private Integer duration;
	private Integer status;
	
	private Integer monday;
	private Integer tuesday;
	private Integer wednesday;
	private Integer thursday;
	private Integer friday;
	private Integer saturday;
	private Integer sunday;
	private Integer downtime;
	
	private Date mondayfrom;
	private Date mondayto;
	
	private Date tuesdayfrom;
	private Date tuesdayto;
	
	private Date wednesdayfrom;
	private Date wednesdayto;
	
	private Date thursdayfrom;
	private Date thursdayto;
	
	private Date fridayfrom;
	private Date fridayto;
	
	private Date saturdayfrom;
	private Date saturdayto;
	
	private Date sundayfrom;
	private Date sundayto;
	
	private Date downtimefrom ;
	private Date downtimeto;
	
	
	
	public Schedule(Date from, Date to, int level,int duration, int status, int monday,int tuesday, int wednesday, int thursday, int friday, int saturday, int sunday, int downtime, Date monFrom, Date monTo, Date TueFrom, Date TueTo, Date wedFrom, Date wedTo, Date thuFrom, Date  thuTo, Date friFrom, Date friTo, Date satFrom, Date satTo, Date sunFrom, Date sunTo, Date downFrom, Date downTo) {
		
		setDtfrom(from);
		setDtto(to);
		setLevel(level);
		setDuration(duration);
		setStatus(status);
		
		setMonday(monday);
		setTuesday(tuesday);
		setWednesday(wednesday);
		setThursday(thursday);
		setFriday(friday);
		setSaturday(saturday);
		setSunday(sunday);
		setDowntime(downtime);
		
		setMondayfrom(monFrom);
		setMondayto(monTo);
		
		setTuesdayfrom(TueFrom);
		setTuesdayto(TueTo);
		
		setWednesdayfrom(wedFrom);
		setWednesdayto(wedTo);
		
		setThursdayfrom(thuFrom);
		setThursdayto(thuTo);
		
		setFridayfrom(friFrom);
		setFridayto(friTo);
		
		setSaturdayfrom(satFrom);
		setSaturdayto(satTo);
		
		setSundayfrom(sunFrom);
		setSundayto(sunTo);
		
		setDowntimefrom(downFrom);
		setDowntimeto(downTo);		
		
	}
	
	public Schedule()
	{
	
	}

	public void setSprinklerid(String sprinklerid) {
		this.sprinklerid = sprinklerid;
	}

	public String getSprinklerid() {
		return sprinklerid;
	}

	public void setMonday(Integer monday) {
		this.monday = monday;
	}

	public Integer getMonday() {
		return monday;
	}

	public void setTuesday(Integer tuesday) {
		this.tuesday = tuesday;
	}

	public Integer getTuesday() {
		return tuesday;
	}

	public void setWednesday(Integer wednesday) {
		this.wednesday = wednesday;
	}

	public Integer getWednesday() {
		return wednesday;
	}

	public void setThursday(Integer thursday) {
		this.thursday = thursday;
	}

	public Integer getThursday() {
		return thursday;
	}

	public void setFriday(Integer friday) {
		this.friday = friday;
	}

	public Integer getFriday() {
		return friday;
	}

	public void setSaturday(Integer saturday) {
		this.saturday = saturday;
	}

	public Integer getSaturday() {
		return saturday;
	}

	public void setDowntime(Integer downtime) {
		this.downtime = downtime;
	}

	public Integer getDowntime() {
		return downtime;
	}

	public void setSunday(Integer sunday) {
		this.sunday = sunday;
	}

	public Integer getSunday() {
		return sunday;
	}

	public void setMondayfrom(Date mondayfrom) {
		this.mondayfrom = mondayfrom;
	}

	public Date getMondayfrom() {
		return mondayfrom;
	}

	public void setMondayto(Date mondayto) {
		this.mondayto = mondayto;
	}

	public Date getMondayto() {
		return mondayto;
	}

	public void setTuesdayfrom(Date tuesdayfrom) {
		this.tuesdayfrom = tuesdayfrom;
	}

	public Date getTuesdayfrom() {
		return tuesdayfrom;
	}

	public void setTuesdayto(Date tuesdayto) {
		this.tuesdayto = tuesdayto;
	}

	public Date getTuesdayto() {
		return tuesdayto;
	}

	public void setWednesdayfrom(Date wednesdayfrom) {
		this.wednesdayfrom = wednesdayfrom;
	}

	public Date getWednesdayfrom() {
		return wednesdayfrom;
	}

	public void setWednesdayto(Date wednesdayto) {
		this.wednesdayto = wednesdayto;
	}

	public Date getWednesdayto() {
		return wednesdayto;
	}

	public void setThursdayfrom(Date thursdayfrom) {
		this.thursdayfrom = thursdayfrom;
	}

	public Date getThursdayfrom() {
		return thursdayfrom;
	}

	public void setThursdayto(Date thursdayto) {
		this.thursdayto = thursdayto;
	}

	public Date getThursdayto() {
		return thursdayto;
	}

	public void setFridayfrom(Date fridayfrom) {
		this.fridayfrom = fridayfrom;
	}

	public Date getFridayfrom() {
		return fridayfrom;
	}

	public void setFridayto(Date fridayto) {
		this.fridayto = fridayto;
	}

	public Date getFridayto() {
		return fridayto;
	}

	public void setSaturdayfrom(Date saturdayfrom) {
		this.saturdayfrom = saturdayfrom;
	}

	public Date getSaturdayfrom() {
		return saturdayfrom;
	}

	public void setSaturdayto(Date saturdayto) {
		this.saturdayto = saturdayto;
	}

	public Date getSaturdayto() {
		return saturdayto;
	}

	public void setSundayfrom(Date sundayfrom) {
		this.sundayfrom = sundayfrom;
	}

	public Date getSundayfrom() {
		return sundayfrom;
	}

	public void setSundayto(Date sundayto) {
		this.sundayto = sundayto;
	}

	public Date getSundayto() {
		return sundayto;
	}

	public void setDowntimefrom(Date downtimefrom) {
		this.downtimefrom = downtimefrom;
	}

	public Date getDowntimefrom() {
		return downtimefrom;
	}

	public void setDowntimeto(Date downtimeto) {
		this.downtimeto = downtimeto;
	}

	public Date getDowntimeto() {
		return downtimeto;
	}

}
