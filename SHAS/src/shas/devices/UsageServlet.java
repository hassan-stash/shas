...
public class UsageServlet extends HttpServlet {
	...
	//-------------------------------------------------------------------//
	// Get the usage report for the given date range
	//
	//-------------------------------------------------------------------//
	void GetUsageReport(HttpServletRequest request, HttpServletResponse response) throws SQLException
	{
		...
		Hashtable<String,Integer> objRs = objService.GetUsage("power", dtfromdate, dttodate); 				
		context.setAttribute("rsPower", objRs);
		SetFields(request,response);		
